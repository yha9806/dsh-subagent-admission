...
`refresh-baseline.mts` runs `git ls-remote`, reads the official source package version from the exact fetched commit, queries npm dist-tags, and resolves Discussion #131 without a token from two direct GitHub surfaces. The official REST discussion and paginated comment endpoints provide the numeric discussion ID, state, total comment count, `updated_at`, and every comment's `author_association`; maintainer comments are exactly associations `OWNER`, `MEMBER`, or `COLLABORATOR`. The canonical public HTML provides only the discussion upvote count: locate exactly one button whose id is `discussion-upvote-button-Discussion-${discussion.id}` and whose `aria-label` is exactly `Upvote: N`, so comment upvote buttons cannot be confused with the discussion. Require the fetched comment-row count to equal the REST discussion count. A missing, duplicate, malformed, paginated, or cross-source-inconsistent shape fails closed instead of guessing from nearby rendered text. Record both direct source URLs in the baseline. It writes stable sorted JSON with an explicit `observedAt`. `--check` ignores only the passage of wall-clock time, compares all source values with `compatibility/baseline.json`, and exits nonzero on drift. Refresh preserves verified `strictTargets` byte-for-byte, but marks them non-current when observed source identity moves; it never promotes a new target. It must produce the current `source-npm-diverged` state rather than converting rc.5 source into rc.6.
