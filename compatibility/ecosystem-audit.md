...
- Delivery checkpoint: 2026-08-14, after the public source push
...
gate timestamp—so broad “first plugin” claims are not credible. Four current
projects are especially relevant.

### 2.1 `Nunchakus888/dsh-turn-budget`

- Exact commit: `9583ad34682455a0d9be3bc35ec809908e21d1d2`
- Package: `dsh-turn-budget@0.1.0`
- Product: an immediately installable, fail-closed per-turn circuit breaker for
  step count, tool-call count, and observed provider-token use.
- Integration: public `agent/pre-step` and `tools/pre-execute` hooks; no DSH
  source patch.

Direct source shows that it validates configured budgets, tracks one Agent
turn, rejects the next step or tool execution before more work, and fails
closed when required budget state cannot be established. The author explicitly
states in Discussion #131 that it does not solve cross-subagent N-squared
growth or process-wide `maxConcurrent` / `maxTotal` admission.

This is the closest immediate operational response to #131 and must not be
hidden behind a generic API-result count. It also sharpens the residual scope:

- `dsh-turn-budget` bounds work *inside one turn* through documented public
  hooks and can be installed on stock DSH now;
- this project experiments with atomic *materialisation admission* shared by
  all runtime callers, global/root active ownership, cold resume, and quiescent
  release;
- the two controls are complementary and composable, not substitutes and not a
  “better/worse” hierarchy.

### 2.2 `PerryLink/dsh-background-agents`
...
### 2.3 `NanmiCoder/dsh-agent-teams`
...
- Existing DSH plugins: per-tool/per-team limits, own-tool race serialisation,
  per-turn step/tool/token circuit breakers, continuable catalogues, dependency
  gates, restart recovery, and mature GUIs.
...
`dsh-turn-budget` is the immediate stock-runtime circuit breaker;
`dsh-subagent-admission` is an experimental shared lifecycle authority. The
public story should lead with that complement rather than superiority.
...
- `perRootAdmittedTotal=24` is a mandatory, monotonic lifetime fuse in v0.1. It
  has no refund, disable, window, or reset; a sufficiently long-lived root is
  permanently exhausted. Product semantics must later separate active caps,
  optional lifetime fuses, epoch/window budgets, and audited offline migration.
